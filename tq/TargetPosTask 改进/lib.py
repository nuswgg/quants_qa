...
class TargetPosTask(object, metaclass=TargetPosTaskSingleton):
    ...
    def __init__(self, api: TqApi, symbol: str, price: str = "ACTIVE", offset_priority: str = "今昨,开",
        ...
        **注意:**
        ...
        Args:
            ...
            trade_chan (TqChan): [可选]成交通知channel, 当有成交发生时会将最新成交trade发到该channel上
        """
        ...
        self._trade_response_function = None
        self._trade_update_task = self._api.create_task(self._update_trade())
    ...
    def is_finished(self):
        ...
        当前target_volume结束与否
        """
        return self._target_pos is None

    def set_target_volume(self, volume: int, trade_response_function: Callable[[Trade], None] = None) -> None:
        """
        设置目标持仓手数

        Args:
            volume (int): 目标持仓手数，正数表示多头，负数表示空头，0表示空仓
            trade_response_function (Callable):成交响应函数 ->输入值为Trade
        Example::
            ...
            while True:
                ...
                api.wait_update()
        """
        if trade_response_function is not None and callable(trade_response_function) and self.is_finished:
            # 如果目前task正在运行->则当前成交响应句柄不能被覆盖(逻辑纯洁性)
            self._trade_response_function = trade_response_function
    ...
    async def _update_trade(self):
        """监听成交更新
        当目标持仓完成时 关闭成交响应句柄
        """
        async for _ in self._trade_chan:
            if self._target_pos == self._pos.pos:  # 关闭目标持仓
                self._target_pos = None
            if callable(self._trade_response_function):  # 响应成交句柄
                self._trade_response_function(_)
            if self.is_finished:  # 关闭响应句柄
                self._trade_response_function = None
    ...
    async def _target_pos_task(self):
        ...
        try:
            async for target_pos in self._pos_chan:
                ...
                self._target_pos = target_pos  # 有任务时有值
                # 确定调仓增减方向
        ...
        async with self._api.register_update_notify() as update_chan:
            ...
            while order.status != "FINISHED" or (order.volume_orign - order.volume_left) != sum(
                ...
                if order.volume_left != last_left:
                    ...
                    await self._trade_chan.send(order.trade_last)
                if order != last_order:
