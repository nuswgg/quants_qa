...
class DonMA(object):
    ...
    def __init__(self, symbols: list, account=None, window_ma=5, window_hl=5, market_cap=1e6, cost_percentage=1,
                 backtest=True, debug=False, kq=None, tq_chan=None):
        ...
        if backtest:
            self.api = TqApi(self.account, backtest=TqBacktest(start_dt=date(2020, 7, 28), end_dt=date(2020, 8, 3)),
                             web_gui=True)
        elif kq != None:
            self.api = kq
        ...
        for symbol in self.symbols:
            ...
            self.states[symbol] = {'position': cloud_pos, "last_price": cloud_last_pricce, 'pendant_coef': 1,
                                   'extreme_since_entry': cloud_last_pricce, 'open_ma': cloud_last_pricce}
    ...
    def save_to_json(self):
        ...
        for i in self.states:
            output_dict[i] = {'pendant_coef': self.states[i]['pendant_coef'],
                              'extreme_since_entry': self.states[i]['extreme_since_entry'],
                              'open_ma': self.states[i]['open_ma']}
    ...
    def recalc_parameter(self, s: str):
        ...
        custom_logger.warning("Don-chian {} upper middle low: {}, {}, {}".format(symbol, str(self.channel_up[symbol]),
                                                                                 str(self.ma[symbol]),
                                                                                 str(self.channel_down[symbol])))
        return True
                ...
                if self.api.is_changing(self.quote[s], 'last_price'):
                    ...
                    if self.states[s]['position'] == 0 and not self.t_0trades[s]:
                        ...
                        if curr_price == self.ma[s]:
                            ...
                            op_quantity = min([(self.market_cap * self.cost_percentage) / (
                                        abs(curr_price - self.ma[s]) * self.units[s]),
                                               self.market_cap / (self.quote[s].last_price * self.units[s])])
                                    ...
                                    if pendant_boundary > actual_ma:
                                        if (self.states[s]['position'] >= 3) and not self.pendant_trades[s]:
                                            ...
                                            custom_logger.warning(
                                                'pos change: {}, {}'.format(self.states[s]['position'],
                                                                            self.states[s]['position'] - (
                                                                                int(self.states[s]['position'] / 3))))
                                            self.set_position(s, self.states[s]['position'] - (
                                                int(self.states[s]['position'] / 3)), True)
...
if __name__ == "__main__":
    ...
    for i in donma.symbols:
        custom_logger.warning(
            "curr active: %s pos: %d, open c: %f,  open ma: %f, pendant coef: %f, extreme since entry: %f" % (
            i, donma.states[i]['position'], donma.states[i]["last_price"], donma.states[i]['open_ma'],
            donma.states[i]['pendant_coef'], donma.states[i]['extreme_since_entry']))
